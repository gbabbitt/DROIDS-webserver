...
for (my $i = 0; $i < scalar @IN2; $i++){
  ...
  for (my $j = 0; $j < scalar @IN; $j++){
    ...
    #$chain = @inrow[4];
    $res_pos = @inrow[4];
    if($chainID eq "A" && $res_pos == $res_pos1){print "$atom_pos\t"."$res_pos1\t"."$res_type1\t"."$KL\t"."$p_value\t"."$chainID\n";}
    if($chainID eq "A" && $res_pos == $res_pos1){print OUTFILE "$atom_pos\t"."$res_pos1\t"."$res_type1\t"."$KL\t"."$p_value\t"."$chainID\n";}
